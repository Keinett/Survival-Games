...
public class CommandHandler implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd1, String commandLabel, String[] args) {
        ...
        if (cmd1.getName().equalsIgnoreCase("join")) {
            try {
                commands.get("join").onCommand(player, args);
                return true;
            } catch (Exception e) {
                msgmgr.sendMessage(PrefixType.INFO, "Incorrect usage! Correct usage /join <Number>", player);
                msgmgr.sendMessage(PrefixType.INFO, "Type /sg help for command information", player);
            }
        }
        if (cmd1.getName().equalsIgnoreCase("vote")) {
            try {
                commands.get("vote").onCommand(player, args);
                return true;
            } catch (Exception e) {
            }
        }
        if (cmd1.getName().equalsIgnoreCase("spectate")) {
            try {
                commands.get("spectate").onCommand(player, args);
                return true;
            } catch (Exception e) {
                msgmgr.sendMessage(PrefixType.INFO, "Incorrect usage! Correct usage /spectate <Number>", player);
                msgmgr.sendMessage(PrefixType.INFO, "Type /sg help for command information", player);
            }
        }
        if (cmd1.getName().equalsIgnoreCase("lobby")) {
            try {
                commands.get("join").onCommand(player, args);
                return true;
            } catch (Exception e) {
        ...
        if (cmd1.getName().equalsIgnoreCase("games")) {
            try {
                commands.get("games").onCommand(player, args);
                return true;
            } catch (Exception e) {
        ...
        if (cmd1.getName().equalsIgnoreCase("leave")) {
            try {
                commands.get("leave").onCommand(player, args);
                return true;
            } catch (Exception e) {
